...
The private CsmIdentity /personal-gift-contacts route authenticates the actual shared session and requires CSM Giving read plus HS Contacts read access. It returns a bounded search result or one selected contact. Donor search accepts field=email (default), name, or phone and searches only that field, never digits extracted from an email against phone numbers. Common phone punctuation is ignored. CSM displays these choices as radio buttons; changing the choice clears the previous donor selection. Public APIs cannot impersonate an identity. CSM mutations require Giving edit, CSRF, and an allowed origin. Evidence uses private LEDGER_ATTACHMENTS storage with a 5 MB limit and PNG, JPEG, or PDF signature checks.
...
| 2026-09-23 | 4.10 correction | Donor search restricted to the selected Email, Name, or Phone number field, with regression coverage for email digits matching unrelated phones. |
