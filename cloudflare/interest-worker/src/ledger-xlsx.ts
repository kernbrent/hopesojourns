...
export function buildLedgerWorkbook(entries: LedgerWorkbookRow[]): Uint8Array {
  const headers = [
    ...
    "Budget Category", "Purpose", "Charitable Amount", "Trip ID", "Trip Account ID", "Funding Source ID",
    "Check Number", "Note", "Receipt Count", "Source", "Source File", "Source Row", "Currency", "Gross", "Fee", "Net", "Created At",
  ];
  ...
  for (const entry of entries) {
    ...
    ledgerRows.push([
      ...
      { value: entry.transactionPurpose },
      { value: entry.charitableAmount, style: 2, numeric: true },
      { value: entry.tripId },
      { value: entry.tripAccountId },
  ...
  const files = [
    ...
    { name: "xl/worksheets/sheet1.xml", bytes: encoder.encode(worksheetXml(ledgerRows, [38, 13, 16, 18, 22, 14, 24, 38, 20, 24, 18, 38, 38, 38, 14, 42, 14, 12, 28, 12, 10, 14, 14, 14, 24])) },
