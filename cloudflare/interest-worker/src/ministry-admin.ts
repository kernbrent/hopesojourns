...
async function inboxItems(env:AdminEnv,user:MmtIdentity):Promise<Row[]> {
 ...
 env.DB.prepare(`SELECT id,title,updated_at AS created_at, CASE WHEN budget_completed_at IS NULL AND status NOT IN ('archived','canceled','completed') AND NOT EXISTS(SELECT 1 FROM trip_reviews r WHERE r.trip_id=trips.id AND r.section='budget' AND r.status='not_applicable' AND NOT EXISTS(SELECT 1 FROM trip_cost_items c WHERE c.trip_id=trips.id AND c.payment_status!='canceled' AND (c.actual_total>0 OR c.estimated_total>0)) AND NOT EXISTS(SELECT 1 FROM trip_charges c WHERE c.trip_id=trips.id AND c.status NOT IN ('canceled','waived')) AND NOT EXISTS(SELECT 1 FROM trip_payments p WHERE p.trip_id=trips.id AND p.status='received')) THEN 'action' ELSE 'completed' END AS status FROM trips ORDER BY updated_at DESC LIMIT 100`).all<Row>()]);
