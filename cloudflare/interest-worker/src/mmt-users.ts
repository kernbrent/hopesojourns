import {accountEmailReady,sendAccountEmail} from './account-email';
...
export function publicUser(u:MmtIdentity){return {id:u.id,username:u.username,first_name:u.first_name,last_name:u.last_name,email:u.email,phone:u.phone,country:u.country,is_admin:!!u.is_admin,permissions:permissions(u),status:u.status,must_change_password:!!u.must_change_password,registered_at:u.registered_at,last_login_at:u.last_login_at??null,revision:u.revision};}
async function user(env:AdminEnv,id:string){const u=await env.DB.prepare('SELECT * FROM mmt_users WHERE id=? AND deleted_at IS NULL').bind(id).first<User>();if(!u)throw new AdminError(404,'NOT_FOUND','User not found.');return u;}
...
export async function handleAccountRequest(request:Request,env:AdminEnv,path:string){
 ...
 if(path==='/admin/account/users'&&request.method==='GET'){const rows=await env.DB.prepare('SELECT * FROM mmt_users WHERE deleted_at IS NULL ORDER BY username').all<User>();const requests=await env.DB.prepare('SELECT * FROM mmt_access_requests ORDER BY created_at DESC LIMIT 250').all();const emailEvents=await env.DB.prepare('SELECT * FROM mmt_email_events ORDER BY created_at DESC LIMIT 100').all();return adminJson({users:rows.results.map(publicUser),requests:requests.results,emailEvents:emailEvents.results,sections,emailReady:accountEmailReady(env)});}
 ...
 if(match){const u=await user(env,match[1]);
 if(request.method==='DELETE'&&!match[2]){
  ...
  if(b.confirmUsername!==u.username)throw new AdminError(422,'CONFIRM_DELETE','Type the user name exactly to confirm deletion.');
  if(Number(b.revision)!==u.revision)throw new AdminError(409,'STALE_EDIT','This user changed. Refresh before deleting.');
  const time=now();
  ...
  const deleted=await env.DB.prepare('SELECT deleted_at FROM mmt_users WHERE id=?').bind(u.id).first<{deleted_at:string|null}>();
  if(deleted?.deleted_at!==time)throw new AdminError(409,'STALE_EDIT','This user changed. Refresh before deleting.');
  await auditStatement(env,'mmt_user',u.id,'deleted').run();
  return adminJson({message:'User deleted. Sign-in access is removed; past activity is preserved.'});
 }
